...
The config has exactly three logical store roles: `stores.capture`, `stores.control`, and `stores.routes`. They may name the same physical R2 bucket. The `MILK_CAPTURE_STORE_*`, `MILK_CONTROL_STORE_*`, and `MILK_ROUTE_STORE_*` bindings remain explicit, and each command opens only its declared roles.
