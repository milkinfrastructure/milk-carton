...
`teacher.max_decisions` is required and must be between 1 and 4,096. It limits permanent teacher decision reservations across the full tenant/project/environment/workload scope, including skipped, failed, expired, and prior-provider work. At the limit, ticks stop creating teacher claims but continue repair, reconciliation, terminalization, and teardown. `status` schema v2, containing records schema v5, reports configured, claimed, and remaining decisions.
