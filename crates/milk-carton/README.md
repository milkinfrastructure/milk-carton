...
Before creating a teacher-run, student train-merge, or three-branch fanout claim, the gateway conditionally creates a strict typed `milk.gpu-launch-intent.v1`. The intent contains the canonical claim and a `milk.gpu-launch-outbox.v1` or `milk.gpu-launch-outbox.v2`, reserves a slot in the same scope-wide allowance as `frontier/gpu-launch/` pointers, and is compare-and-swap terminalized only after the claim, outbox, and required pointer are verified. A successor tick repairs pending intents independently of the current teacher binding or student reservation. Expired or already-terminal work keeps its permanent claim and outbox but no active pointer; the bounded intent record is then removed.

Teacher config names the two student authorities directly: `student_train_runtime_image_reference` is exclusive to train/merge, and `student_branch_runtime_image_reference` is exclusive to the three evaluation branches. Both are digest-only references and both enter the student job ID. Train launch outboxes use only the train image; fanout outboxes use only the branch image. The 7,200 GPU-second total cap remains 1,800 seconds for train plus 1,800 seconds for each of three branches. Stage-tagged GPU evidence prevents the native Prime-RL/CUDA 12.8 train image from claiming the vLLM/CUDA 12.9 FP8 branch runtime.

Each request-key record binds a non-nil `key_id` and exact `api_key_sha256` to a stable non-nil `scope_id`. Multiple rotatable keys may bind the same scope. Baseline-only `serve` configurations with `route: null` may contain multiple scopes; capture and statistics remain under the authenticated key's scope, while outcome ingestion is disabled because its shared key cannot identify a scope. Every control command and every routed deployment remains single-scope. Capture, control, and route objects live under `milk/v1/scopes/<scope_id>/`; eval revisions are data within the scope, not traffic namespaces. Repeated ticks drain all admitted work. Per-job call, concurrency, byte, and execution-time bounds contain failed work; they are not scope-wide quotas.
...
Provider dispatch is outside this repository. Milk Man invokes a fixed one-shot jobs process that reads the Carton control store without write authority, consumes only `frontier/gpu-launch/` pointers, verifies the full frontier -> outbox -> claim -> admitted-image chain, and executes the named teacher, train, or evaluation job. Intent records are Carton-internal recovery state. Jobs must not invoke this binary's `tick` command or share its control-store writer credential.

`prepare-route-proposal --proposal <json> --manifest <output>` consumes key-sorted compact `milk.unsigned-route-proposal.v3` JSON plus one trailing LF from the deterministic harness. Before emitting canonical `milk.route.v1` for external Ed25519 signing, Carton re-reads the content-addressed source, summary, readiness, eval, accepted validation, qualified candidate score, proposal, and current proposal pointer; any scope, profile, series, code, config, digest, candidate, or live-predecessor mismatch fails closed. Candidate proposals route Chat Completions and streaming only; Responses stays on baseline until separately qualified. `--zero` requires the verified live candidate for the exact same proposal and emits a baseline-only successor. `publish-route` verifies the signature and activates either route through the route-store compare-and-swap pointer; the harness never receives the signing key.

Operator routes require only `signing_public_key_hex`, `signing_key_id`, `allow_private_candidate_http`, and `candidate_max_in_flight` in `route`.
...
Copy `deploy/milk-carton-config.example.json`, replace the traffic and outcome key material, and set one or three owner-only Local roots. `traffic_keys` contains 1 to 64 strict `{key_id, api_key_sha256, scope_id, capture_allowed}` records. Key rotation adds a new key ID mapped to the existing scope; an unrouted baseline capture service may also map keys to distinct scopes. An optional `revocation` object containing `revoked_at` and optional `reason` disables a retained record. Set `capture_allowed: true` only for inputs admitted to that scope. Deployment, canary, and ordinary synthetic test keys use `false`. Generate one request key without printing it; the command returns the `key_id` and hash for the record:
