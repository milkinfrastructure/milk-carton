...
impl Records {
    ...
    pub(crate) async fn ingest_student_train_execution(
        ...
        self.store
            .ingest_student_train_execution(scope, result, upload, artifacts)
            .await
    }
    ...
    pub(crate) async fn ingest_student_branch_execution(
        ...
        self.store
            .ingest_student_branch_execution(scope, result, upload, artifacts)
            .await
    }
...
impl RecordStore {
    ...
    async fn ingest_student_train_execution(
        ...
        validate_student_train_upload(scope, &student_job_id, &result, &upload, artifacts.files())?;
...
fn validate_student_stage_gpu_evidence(
    ...
    let device_valid = evidence.mode == StudentGpuMode::H100
        && valid_student_gpu_uuid(uuid)
        && evidence.device.name.contains("H100");
    let runtime_valid = match (&evidence.runtime, expected_variant) {
        (
...
mod tests {
    ...
    fn student_gpu_evidence_is_bound_to_its_train_or_branch_runtime() {
        ...
        validate_student_stage_gpu_evidence(&student_job_id, &recipe_sha256, &train, None, false)
            .unwrap();
        assert!(
            validate_student_stage_gpu_evidence(
                &student_job_id,
